...
| URL 特征 | 路由到 | 原因 |
|----------|--------|------|
| `mp.weixin.qq.com` | 内置 `scripts/fetch_weixin.py` | 公众号有反爬，需 Playwright 抓取。**服务器IP大概率触发验证码**，见下方⚠️ |
...
脚本实际路径：
```bash
python3 SKILL_DIR/scripts/fetch_weixin.py "WEIXIN_URL"
```
...
```bash
python3 SKILL_DIR/scripts/fetch_feishu.py "FEISHU_URL"
```
...
### 公众号文章
```bash
python3 SKILL_DIR/scripts/fetch_weixin.py "https://mp.weixin.qq.com/s/abc123"
```

### 飞书文档
```bash
python3 SKILL_DIR/scripts/fetch_feishu.py "https://xxx.feishu.cn/docx/xxxxxxxx"
```

### 飞书知识库
```bash
python3 SKILL_DIR/scripts/fetch_feishu.py "https://xxx.feishu.cn/wiki/xxxxxxxx"
```
...
### ⚠️ 公众号抓取的现实限制（2026-05 验证）

微信公众号反爬是 **IP 级别** 的。以下方法在服务器IP上全部会被验证码拦截：
- Jina Reader（`r.jina.ai`）→ 返回"环境异常，完成验证后即可继续访问"
- Playwright 脚本（`fetch_weixin.py`）→ `#js_content` selector 超时
- curl + 任意 User-Agent（手机/桌面）→ 空白验证页面
- Google 缓存 / archive.org → 超时或无结果

**不是工具问题，是微信策略问题。** 当脚本返回"环境异常"或 `#js_content` 超时，说明被拦了。

**Fallback 流程：**
1. 先试脚本一次（有时短时间窗口能过）
2. 失败 → 告诉用户"被验证码拦了，帮我粘贴全文或截图"
3. **不要反复重试浪费时间**，一次失败就够了，换方法也不会成功
4. 如果用户坚持要"解决"，诚实说明是IP级别封锁，自动化确实绕不过

### 飞书文档抓取注意事项
- 需要 `FEISHU_APP_ID` + `FEISHU_APP_SECRET` 环境变量
- Wiki 页面需要先解析实际文档 ID，脚本会自动处理
- 飞书也有频率限制，批量抓取时注意间隔
