...
> ⚠️ **核心原则**：直接展示给用户的最终内容——必须 markdown 格式 + 排版好 + **普通人能看得懂的人话**。
...
默认输出 `title`（中文 normalize 过的）。`title_en` 只在用户明确要求英文版或 `title` 为空时才用。

## 常见错误处理

- `No daily report available yet.`（HTTP 404）：当天日报还没生成（北京时间 08:00 之前）。建议拉昨天日报
- HTTP 429（限流）：串行调用 + 翻页加 200ms 间隔即可

## 不要做

- 不要把"今天 AI 圈"等宽问题路由到 daily——默认走 `mode=selected + since=<语义窗>`
- 不要在用户没说"全部"时默认走 `mode=all`
- 不要试图猜测 / 编造内容——永远以 API 返回为准
- 不要在用户输出里暴露端点路径 / raw 参数 / 限流 / 缓存 / cursor 等基础设施细节
- 不要在压缩输出时丢掉每条的 url——用户看到没 URL 的条目会认为不可信
- 不要把"端点路径 / 调用细节"作为输出的引用源——引用源就写 `<source>` 名称
